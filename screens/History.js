 ...
 useEffect(() => {
  ...
  const LeftSwipeActions = (key) => {
    return (
      ...
      onPress={() => {
        setOpenCardIndex(-1); // close the opened card
        // delete the card or perform any other action here
      }}>
  ...
  return (
    ...
    <View style={styles.view4}>
     ...
     <Text style={styles.text4} >Welcome {Namee.split(' ').map((word) => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}</Text>
       ...
       data.slice(0).reverse().map((item,key) => (
        ...
        renderRightActions={LeftSwipeActions}>
          <View style={styles.view}>
            <Card style={styles.cardWrapper}  onPress={() => setOpenCardIndex(key)}>
              ...
              <View style={styles.card}>
                ...
                {openCardIndex === key && LeftSwipeActions(key)}
              </View>
