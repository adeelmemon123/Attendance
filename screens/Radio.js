...
import {KeyboardAvoidingView,Pressable,Alert,BackHandler,View,StyleSheet ,Text,ScrollView,Dimensions,TouchableOpacity,PermissionsAndroid,ActivityIndicator,ImageBackground } from 'react-native';
...
const openCamera = async function() {
  ...
  Alert.alert('Click Picture!','Please take a picture')
...
const options = {
  container: {
    backgroundColor:isFirstLogin ? "white" : active ? "#FFD6D5":"#E9FCE9",
    padding: 5,
    borderRadius: 5,
    alignItems: 'center',
  },
  ...
  return (
    ...
    <Background style={{backgroundColor:isFirstLogin ? "#0096FF" : active ? "red":"#1E9E1E"}}>
    ...
    <View style={styles.dp2}>
        <Text style={{ fontSize: 12, fontWeight:"bold",color:'black'}}>Your Location</Text>
      ...
      {isFirstLogin ?<Text style={styles.text19}>Tap to Check In</Text>:active ?<Text style={styles.text9}>Tap to Check Out</Text>:<Text style={styles.text10}>Tap to Checked In</Text>}</View>
     <View style={{flexDirection:'column',marginTop:height*.01,alignItems: 'center'}}>
  
     <TouchableOpacity   onPress={() => handleClick()}
     style={{backgroundColor: isFirstLogin ? "#E9DCC9": active ? "red":"#1E9E1E",width:90,height:90,justifyContent: 'center',alignItems: 'center',padding: 10,borderRadius: 100} }  >  
    {loading ? (
                  <ActivityIndicator size="large" color="#000"  />
                ) : (
                  isFirstLogin ? <Icon name='md-shield-checkmark-outline' size={30} style={styles.icon4} />: active ?<Icon name='power' size={30} style={styles.icon4} />:<Icon name='ios-checkmark-done-outline' size={30} style={styles.icon4}/>
                )}
